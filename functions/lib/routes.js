...
router.post('/orders', (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    let { currency, items, email, shipping } = req.body;
    try {
        let order = yield orders.create(currency, items, email, shipping);
...
router.post('/orders/:id/pay', (req, res, next) => __awaiter(this, void 0, void 0, function* () {
    let { source } = req.body;
    try {
        // Retrieve the order associated to the ID.
        let order = yield orders.retrieve(req.params.id);
        // Verify that this order actually needs to be paid.
        if (order.metadata.status === 'pending' ||
        ...
        if (source && source.status === 'chargeable') {
            ...
            order = yield orders.update(order.id, { metadata: { status } });
        }
...
router.post('/webhook', (req, res) => __awaiter(this, void 0, void 0, function* () {
    ...
    if (object.object === 'source' &&
        ...
        const order = yield orders.retrieve(source.metadata.order);
        // Verify that this order actually needs to be paid.
        if (order.metadata.status === 'pending' ||
            order.metadata.status === 'paid' ||
        ...
        yield orders.update(order.id, { metadata: { status } });
    }
    ...
    if (object.object === 'charge' &&
        ...
        // Find the corresponding order this source is for by looking in its metadata.
        const order = yield orders.retrieve(charge.source.metadata.order);
        // Update the order status to mark it as paid.
        yield orders.update(order.id, { metadata: { status: 'paid' } });
    }
    ...
    if ((object.object === 'source' || object.object === 'charge') &&
        ...
        if (source.metadata.order) {
            ...
            yield orders.update(order.id, { metadata: { status: 'failed' } });
        }
    }
...
router.get('/orders/:id', (req, res) => __awaiter(this, void 0, void 0, function* () {
    try {
        return res.status(200).json(yield orders.retrieve(req.params.id));
    }
    catch (err) {
...
router.get('/products', (req, res) => __awaiter(this, void 0, void 0, function* () {
    const productList = yield products.list();
...
router.get('/products/:id', (req, res) => __awaiter(this, void 0, void 0, function* () {
    res.json(yield products.retrieve(req.params.id));
}));
