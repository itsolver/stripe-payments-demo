...
This is our Flask server that handles requests from our Stripe Elements checkout flow.
...
def get_products():
    products = InventoryMngr.list_products()
    if InventoryMngr.products_exist(products):
...
def retrieve_product(product_id):
    return jsonify(InventoryMngr.retrieve_product(product_id))
...
def make_order():
    ...
    try:
        order = InventoryMngr.create_order(currency=data['currency'], items=data['items'], email=data['email'],
...
def pay_order(order_id):
    ...
    order = InventoryMngr.retrieve_order(order_id)
    ...
    if source['status'] == 'chargeable':
        ...
        InventoryMngr.update_order(properties={'metadata': {'status': status}}, order=order)
...
def webhook_received():
    ...
    if data_object['object'] == 'source' \
        ...
        order = InventoryMngr.retrieve_order(source['metadata']['order'])
    ...
    if data_object['object'] == 'charge' \
        ...
        InventoryMngr.update_order(properties={'metadata': {'status': 'paid'}},
    ...
    if data_object['object'] in ['source', 'charge'] and data_object['status'] in ['failed', 'canceled']:
        ...
        if source['metadata']['order']:
            InventoryMngr.update_order(properties={'metadata': {'status': 'failed'}},
