...
from inventory_manager import InventoryMngr
...
class AppTestCase(TestCase):
    ...
    def test_pay_pending_order(self):
        ...
        """
        InventoryMngr.retrieve_order = mock.MagicMock(return_value=mocked_pending_order)
    ...
    def test_pay_paid_order(self):
        ...
        """
        InventoryMngr.retrieve_order = mock.MagicMock(return_value=mocked_paid_order)
        response = self.app.post(f'orders/{mocked_failed_order["id"]}/pay', data=json.dumps({'source': mocked_source}))
        self.assertEqual(response.status_code, 403)
    ...
    def test_pay_created_order(self):
        ...
        """
        InventoryMngr.retrieve_order = mock.MagicMock(return_value=mocked_created_order)
        stripe.Charge.create = mock.MagicMock(return_value=mocked_charge)
        InventoryMngr.update_order = mock.MagicMock(return_value=mocked_created_order)

        response = self.app.post(f'orders/{mocked_failed_order["id"]}/pay', data=json.dumps({'source': mocked_source}))
        InventoryMngr.update_order.assert_called_once_with(properties={'metadata': {'status': 'paid'}},
    ...
    def test_pay_order_failed_to_charge(self):
        ...
        response = self.app.post(f'orders/{mocked_failed_order["id"]}/pay', data=json.dumps({'source': mocked_source}))
        InventoryMngr.update_order.assert_called_once_with(properties={'metadata': {'status': 'failed'}},
                                                           order=mocked_created_order)
    ...
    def test_charge_succeeded_webhook(self):
        ...
        """
        InventoryMngr.update_order = mock.MagicMock(return_value=mocked_paid_order)
    ...
    def test_source_chargeable_webhook_with_already_pending_payment(self):
        ...
        InventoryMngr.retrieve_order = mock.MagicMock(return_value=mocked_pending_order)
        stripe.Charge.create = mock.MagicMock(return_value=mocked_charge)
        InventoryMngr.update_order = mock.MagicMock(return_value=mocked_pending_order)
        stripe.Webhook.construct_event = mock.MagicMock(return_value=mocked_source_chargable_webhook_event)
